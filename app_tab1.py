...
def generate_prompt():
    ...
    character_name = st.text_input("Enter character name: \n\n",key="character_name",value="")
    character_type = st.text_input("What type of character is it? (cat, dog, etc.) \n\n",key="character_type",value="")
    character_persona = st.text_input("What personality does the character have? \n\n",
                                      key="character_persona",value="")
    character_location = st.text_input("Where does the character live? \n\n",key="character_location",value="")

    
    story_premise = st.multiselect("What is the story premise? (can select multiple) \n\n",["Love","Adventure","Nerdy","Intellectual","Comedy","Sci-Fi","Fantasy","Mystery","Horror", "Thriller", "Suspense", "Satir", "Ranchy"],key="story_premise",default=["Adventure","Comedy"])
...
def render_story_tab (text_model_pro: GenerativeModel):
    ...
    st.subheader("Generate New Story")
